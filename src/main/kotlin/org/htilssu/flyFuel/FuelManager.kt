...
class FuelManager {
     ...
     */
    private val playerFlying: MutableSet<UUID> = mutableSetOf()

    /**
     * Thuộc tính getter cho phép truy cập chỉ đọc vào playerFlying từ bên ngoài
     * @return Map chỉ đọc của UUID người chơi và trạng thái bay
     */
    val flyingPlayers: Set<UUID>
        get() = playerFlying.toSet()
    ...
    fun setFlying(player: Player, flying: Boolean) {
        if (flying) {
            playerFlying.add(player.uniqueId)
        } else {
    ...
    fun isFlying(player: Player): Boolean {
        return playerFlying.contains(player.uniqueId)
    }
